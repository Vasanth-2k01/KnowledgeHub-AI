...
export function buildRagPrompt(
  ...
  const context = sources.map(s => `Document: ${s.fileName}\nContent:\n${s.content}`).join("\n\n");
...
// Use citations only for factual claims supported by the retrieved documents.

// - Use only the exact source IDs provided, formatted as [SOURCE_X].
// - Cite logical groups of information, not every sentence.
// - Simple paragraph: cite at the end of the paragraph.
// - Bullet or numbered list with the same source: cite once at the end of the list.
// - Table: cite once for the table or relevant row group.
// - Specific/high-value claim: use an inline citation.
// - Multiple sources: place [SOURCE_X] [SOURCE_Y] at the end of the logical block.
// - Do not repeat the same source unnecessarily.
// - Never use more than 2 sources for a single claim.
// - Never combine sources inside one bracket, e.g. [SOURCE_1, SOURCE_2].
// - Do not cite conversation history or general knowledge.
// - Never invent or modify source IDs.
// - Keep citations immediately after the content they support.

// Examples:

// Paragraph:
// React.js, Node.js, and MongoDB are part of the technology stack. [SOURCE_1]

// Bullet list:
// The frontend technologies include [SOURCE_1]:
// - React.js
// - TypeScript
// - Bootstrap
// - ApexCharts
...
// Multiple sources:
// React.js and Node.js are used across the application. [SOURCE_1] [SOURCE_3]
